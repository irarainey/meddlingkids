"""
Navigation, page-load, access-check, and initial data capture
phases (Phases 1-3 of the analysis pipeline).

Browser session creation is handled by
:class:`~src.browser.manager.PlaywrightManager` at a higher level.
Each public function here performs a single focused step and
returns its result without emitting SSE progress events.  The
caller (``stream.py``) yields progress events inline between
calls so they reach the client in real-time rather than being
batched.
"""
