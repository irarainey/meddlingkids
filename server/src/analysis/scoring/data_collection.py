...
def calculate(
    ...
    log.debug("Data collection scoring input", data={
    ...
    tracking_storage = [
        ...
        if any(
            p.search(item.key)
            for p in tracker_patterns.TRACKING_STORAGE_PATTERNS
        )
    ]
    beacon_requests = [
        r
    ...
    analytics_urls = [
        ...
        if any(
            p.search(r.url)
            for p in tracker_patterns.ANALYTICS_TRACKERS
        )
    ]

    log.debug("Data collection detection", data={
    ...
    if len(tracking_storage) > 0:
        points += 3
        issues.append(
            f"{len(tracking_storage)} tracking-related"
            " storage items"
        )
    ...
    if len(beacon_requests) > 30:
        points += 6
        issues.append(
            f"{len(beacon_requests)} tracking beacons/pixels"
            " detected (extreme)"
        )
    ...
    if len(third_party_posts) > 5:
        points += 3
        issues.append(
            f"{len(third_party_posts)} data submissions"
            " to third parties"
        )
    ...
    log.info("Data collection score", data={
        "points": points,
        "max_points": 18,
    ...
    return analysis.CategoryScore(
        points=points, max_points=18, issues=issues
    )
