 ...
 */
export function preloadAllData(categories: readonly PartnerCategoryConfig[]): void {
  // Load scripts
  getTrackingScripts()
  getBenignScripts()
  
  // Load all partner databases
  for (const config of categories) {
    getPartnerDatabase(config.file)
  }
}
 ...
 */
export function getDataStats(categories: readonly PartnerCategoryConfig[]): {
  trackingPatterns: number
  benignPatterns: number
  partnerEntries: Record<string, number>
  totalPartners: number
} {
  const trackingPatterns = getTrackingScripts().length
  const benignPatterns = getBenignScripts().length
  
  const partnerEntries: Record<string, number> = {}
  let totalPartners = 0
  
  for (const config of categories) {
    const db = getPartnerDatabase(config.file)
    const count = Object.keys(db).length
    partnerEntries[config.file] = count
    totalPartners += count
  }
