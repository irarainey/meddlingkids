/**
 * @fileoverview Prompts for script analysis using LLM.
 */

/** System prompt for script analysis */
export const SCRIPT_ANALYSIS_SYSTEM_PROMPT = `You are a JavaScript security and privacy analyst. Analyze the provided script and explain what it does in 1-2 sentences (max 40 words).
 ...
 * @param content - The script content (may be truncated)
 * @returns User prompt for script analysis
 */
export function buildScriptAnalysisUserPrompt(url: string, content: string): string {
  return `URL: ${url}\n\nScript content:\n\`\`\`javascript\n${content}\n\`\`\``
}
