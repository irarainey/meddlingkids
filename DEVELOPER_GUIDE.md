...
| `LLM Client` | `llm_client.py` | Chat client factory (`SupportsChatGetResponse`) with `deployment_override` support for per-agent model selection, `use_responses_api` for creating `AzureOpenAIResponsesClient` instances (auto-upgrades API version to `2025-03-01-preview` minimum when needed), and Managed Identity support via `DefaultAzureCredential` when `AZURE_USE_MANAGED_IDENTITY=true` |
